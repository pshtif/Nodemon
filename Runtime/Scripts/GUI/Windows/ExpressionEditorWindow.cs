...
namespace Nodemon
{
    public class ExpressionEditorWindow : UniGUIWindow
    {
        ...
        public static ExpressionEditorWindow InitExpressionEditorWindow(Parameter p_parameter)
        {
            if (Instance != null)
            {
                UniGUIWindowManager.Close(Instance);
            }
            Instance = new ExpressionEditorWindow();
            Instance._rect = new Rect(0, 0, 800, 400);
            _parameter = p_parameter;
            UniGUIWindowManager.Show(Instance);
        ...
        public override void DrawGUI(int p_id)
        {
            Debug.Log("DrawGUI");
            if (_parameter == null)
            {
                UniGUIWindowManager.Close(this);
                return;
            }
            ...
            Rect bounds = new Rect(20, 34, GetRect().width - 20, GetRect().height - 34);
